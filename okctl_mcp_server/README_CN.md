...
```json
{
  "mcpServers": {
    "okctl": {
      "command": "uv",
      "args": ["--directory", "<path/to/mcp-oceanbase/okctl_mcp_server>", "run", "okctl_mcp_server"],
      "env": {
...
```bash
uv run --directory <path/to/mcp-oceanbase/okctl_mcp_server> okctl_mcp_server --tools=clusters
```
